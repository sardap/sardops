...
impl Scene for PetInfoScene {
    ...
    fn render(&self, display: &mut GameDisplay, args: &mut RenderArgs) {
        ...
        match self.state {
            State::Main => {
                const Y_BUFFER: i32 = 10;
                let pet = &args.game_ctx.pet;

                display.render_sprite(&self.pet_render);
                {
                    let pet_render = IVec2::new(
                        self.pet_render.pos.x as i32,
                        self.pet_render.pos.y as i32 - self.pet_render.static_image().isize.y + 2,
                    );
                ...
                let mut current_y =
                    self.pet_render.pos.y as i32 + self.pet_render.static_image().isize.y + 5;
                {
                    let str = str_format!(str32, "PID:{:010X}", pet.upid);
                    display.render_text_complex(
                        &IVec2::new(TEXT_X_OFFSET, current_y),
                        &str,
...

                {
                    ...
                    display.render_text_complex(
                        &IVec2::new(TEXT_X_OFFSET + assets::IMAGE_AGE_SYMBOL.isize.x, current_y),
                        &str,
                        ComplexRenderOption::new()
                            .with_white()
                            .with_font(&fonts::FONT_VARIABLE_SMALL),
            ...
            State::LifeStages => {
                    ...
                    .enumerate()
                {
                    ...
                    display.render_text_complex(
                        &IVec2::new(2, current_y),
                        &str,
                        ComplexRenderOption::new()
                            .with_white()
                            .with_font(&fonts::FONT_VARIABLE_SMALL),
                    );
                    current_y += Y_BUFFER + 1 + pet_render.static_image().isize.y / 2;
                    ...
                    current_y += pet_render.static_image().isize.y / 2 + Y_BUFFER;
                }
            }
            State::Parents => {
                const Y_BUFFER: i32 = 6;
                ...
                if let Some(parents) = &args.game_ctx.pet.parents {
                    for (i, parent) in parents.values.iter().enumerate() {
                        ...
                        display.render_image_complex(
                            CENTER_X_I32 - self.parent_renders[i].static_image().isize.x / 2,
                            current_y,
                            self.parent_renders[i].image(),
                            ComplexRenderOption::new().with_white(),
                        );

                        current_y += self.parent_renders[i].static_image().isize.y + Y_BUFFER;
